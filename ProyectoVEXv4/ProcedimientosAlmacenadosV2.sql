...
BEGIN
    SELECT 
        ...
        -- Categorías de Entrenador (Concatenadas)
        ...
        -- Categoría de Juez (JOIN directo)
        -- Si es NULL, devolverá NULL y el PHP mostrará "Sin asignar"
        c_juez.nombre as cat_juez
        
    FROM Asistente a
    LEFT JOIN Juez j ON a.idAsistente = j.idAsistente_Asistente
    LEFT JOIN Entrenador e ON a.idAsistente = e.idAsistente_Asistente
    -- JOIN CLAVE: Unimos Juez con Categoria para obtener el nombre
    LEFT JOIN Categoria c_juez ON j.idCategoria = c_juez.idCategoria
...
-- =================================================================
-- PROCEDIMIENTO FALTANTE: OBTENER JUECES POR CATEGORÍA
-- Este procedimiento busca jueces que pertenezcan a la categoría seleccionada
-- para mostrarlos en la lista de asignación.
-- =================================================================

DROP PROCEDURE IF EXISTS ObtenerJuecesValidosParaCategoria //

CREATE PROCEDURE ObtenerJuecesValidosParaCategoria(IN p_idCategoria INT)
BEGIN
    SELECT 
        j.idJuez, 
        CONCAT(a.nombre, ' ', a.apellidoPat) as nombre_completo, 
        IFNULL(ep.nombreEscuela, 'Sin Escuela') as nombreEscuela,
        CASE 
            WHEN j.idCategoria IS NULL THEN '(General)'
            ELSE '' 
        END as etiqueta_extra
    FROM Juez j
    INNER JOIN Asistente a ON j.idAsistente_Asistente = a.idAsistente
    LEFT JOIN EscuelaProcedencia ep ON j.codEscuela_EscuelaProcedencia = ep.codEscuela
    -- La magia está aquí: Trae los que coinciden O los que están libres (NULL)
    WHERE j.idCategoria = p_idCategoria OR j.idCategoria IS NULL;
END //
