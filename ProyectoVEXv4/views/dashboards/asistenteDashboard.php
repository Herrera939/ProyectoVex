...
<head>
    ...
    <style>
        ...
        .team-name { font-size: 1.3rem; font-weight: bold; color: var(--primary); margin-bottom: 0px; word-break: break-word; overflow-wrap: break-word;}
...
<body>
    ...
    <main class="main-content">
        ...
        <?php if($msg): ?>
            <div class="alert-box <?php echo (strpos($msg, 'Error:') !== false) ? 'alert-error' : 'alert-success'; ?>">
                <i class="fas <?php echo (strpos($msg, 'Error:') !== false) ? 'fa-exclamation-circle' : 'fa-check-circle'; ?>"></i> 
                    ...
                    <div class="team-card">
                        ...
                        <div class="card-actions">
                            <button class="btn-small btn-view" 
                                onclick="verDetallesEquipo(<?php echo $eq['idEquipo']; ?>, '<?php echo htmlspecialchars($eq['nombreEquipo']); ?>')">
    ...
    <div id="modalDetalles" class="modal">
        <div class="modal-content" style="width: 600px;">
            <h2 style="margin-top:0; border-bottom: 2px solid #FDF5A3; padding-bottom:10px;">
                Detalles: <span id="tituloDetalleEquipo" style="color:#2C2C54;"></span>
            </h2>
            
            <div style="margin-top: 20px;">
                <table style="width:100%; border-collapse: collapse; font-size:0.9rem;">
                    <thead style="background:#2C2C54; color:white;">
                        <tr>
                            <th style="padding:10px; text-align:left;">No. Control</th>
                            <th style="padding:10px; text-align:left;">Nombre Completo</th>
                            <th style="padding:10px; text-align:left;">Edad</th>
                            <th style="padding:10px; text-align:left;">Escuela</th>
                        </tr>
                    </thead>
            ...
            <div style="display:flex; justify-content:end; margin-top:20px;">
                <button type="button" onclick="closeModal('modalDetalles')" class="btn-main" style="padding: 8px 20px;">Cerrar</button>
            </div>
        </div>
    </div>
    ...
    <script>
        //Limpia de parametros extra de la URL
        if (window.history.replaceState) {
            // Obtenemos la URL actual
            const url = new URL(window.location.href);
            
            // Verificamos si tiene parámetros de mensaje o error
            if (url.searchParams.has('msg') || url.searchParams.has('error')) {
                
                // Construimos la URL limpia (sin lo que está después del ?)
                const cleanUrl = window.location.protocol + "//" + window.location.host + window.location.pathname;
                
                // Reemplazamos la URL en la barra de direcciones sin recargar
        ...
        // Función para cargar y mostrar los detalles del equipo
        function verDetallesEquipo(idEquipo, nombreEquipo) {
            // 1. Poner el nombre del equipo en el título
            document.getElementById('tituloDetalleEquipo').innerText = nombreEquipo;
            ...
            // 4. Pedir el HTML al servidor (AJAX)
            fetch('../../controllers/tabla_integrantes.php?idEquipo=' + idEquipo)
                .then(response => response.text())
                .then(html => {
                    // Pegar el HTML recibido dentro de la tabla
